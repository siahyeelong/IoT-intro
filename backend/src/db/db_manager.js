...
async function dropTables() {
  ...
  try {
    ...
    let dropTablesQuery = `DROP TABLE IF EXISTS devices CASCADE; `;
    await client.query(dropTablesQuery);

    dropTablesQuery = `DROP TABLE IF EXISTS rooms CASCADE; `;
    await client.query(dropTablesQuery);

    dropTablesQuery = `DROP TABLE IF EXISTS buildings CASCADE; `;
    await client.query(dropTablesQuery);
...
async function createTables() {
  ...
  try {
    ...
    const sensorsTableQuery = `CREATE TABLE IF NOT EXISTS sensors (
      id SERIAL PRIMARY KEY,
      name VARCHAR NOT NULL,
    ...
    await client.query(sensorsTableQuery);
...
async function populateRoomsAndBuildingsTablesIfEmpty() {
  ...
  try {
    ...
    if (nbRooms === 0) {
      ...
      const queryRooms = 'INSERT INTO rooms(name, capacity, building_id) VALUES ($1,$2,$3)';
      for (const room of rooms) {
        await client.query(queryRooms,
          [room.name, room.capacity, room.building_id]);
      }
    } else {
...
export {
  addPostgisToDb, createTables, dropTables, populateRoomsAndBuildingsTablesIfEmpty, updateGeomProjection, query,
};
